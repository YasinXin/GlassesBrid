...
public class AssetBundleInfoLua
{
	public AssetBundle m_AssetBundle;
	public int m_ReferencedCount;

	public AssetBundleInfoLua (AssetBundle assetBundle)
	{
...
public class ResourceLuaManager : Manager
{
	WWW m_download = null;
	bool m_load = false;
	...
	Dictionary<string, AssetBundleInfoLua> m_LoadedAssetBundles = new Dictionary<string, AssetBundleInfoLua> ();
	Dictionary<string, List<LoadAssetRequest>> m_LoadRequests = new Dictionary<string, List<LoadAssetRequest>> ();
	string m_ManifestName = "";
	...
	public void Initialize (string manifestName, string assetBundlePath, Action initOK)
	{
		m_ManifestName = manifestName;
		m_BaseDownloadingURL = assetBundlePath + "/";
		LoadAsset<AssetBundleManifest> (m_ManifestName, new string[] { "AssetBundleManifest" }, delegate(UObject[] objs) {
			if (objs.Length > 0) {
	...
	public void InitializeManifest (string manifestName, string assetBundlePath, Action<AssetBundleManifest> initOK)
	{
		m_ManifestName = manifestName;
		m_BaseDownloadingURL = assetBundlePath + "/";
		LoadAsset<AssetBundleManifest> (m_ManifestName, new string[] { "AssetBundleManifest" }, delegate(UObject[] objs) {
	...
	IEnumerator OnLoadAsset<T> (string abName) where T : UObject
	{
		...
		AssetBundleInfoLua bundleInfo = GetLoadedAssetBundle (abName);
		if (bundleInfo == null) {
	...
	IEnumerator OnLoadAssetBundle (string abName, Type type)
	{
		...
		if (type == typeof(AssetBundleManifest))
			m_download = new WWW (url);
		else {
			string[] dependencies = m_AssetBundleManifest.GetAllDependencies (abName);
			if (dependencies.Length > 0) {
				Util.Log ("abName===>" + dependencies);
				m_Dependencies.Add (abName, dependencies);
				for (int i = 0; i < dependencies.Length; i++) {
					string depName = dependencies [i];
					AssetBundleInfoLua bundleInfo = null;
			...
			m_download = WWW.LoadFromCacheOrDownload (url, m_AssetBundleManifest.GetAssetBundleHash (abName), 0);
		}
		Util.Log ("=====>yield return m_download<====== url===>" + url);
		m_load = true;
		yield return m_download;
		m_load = false;
		AssetBundle assetObj = m_download.assetBundle;
		if (assetObj != null) {
			m_LoadedAssetBundles.Add (abName, new AssetBundleInfoLua (assetObj));
		}
	}
	...
	{
		if (m_load) {
			Util.Log ("m_download progress====>" + m_download.progress);
		}
	}

	AssetBundleInfoLua GetLoadedAssetBundle (string abName)
	{
		AssetBundleInfoLua bundle = null;
		...
		foreach (var dependency in dependencies) {
			AssetBundleInfoLua dependentBundle;
	...
	public void UnloadAssetBundle (string abName,Action initOK, bool isThorough = false)
	{
		abName = GetRealAssetPath (abName);
	...
	{
		AssetBundleInfoLua bundle = GetLoadedAssetBundle (abName);
		if (bundle == null)
			return;
